...
READINESS = [
    ...
    ["Methodology", "Green", "18 tests, 40 rules, 18 validation rules — 28/28 verdicts and 40/40 rules verified."],
...
def main():
    ...
    out1 = os.path.join(ROOT, "NEICS_Walkthrough.html")
    out2 = os.path.join(ROOT, "docs", "NEICS_Walkthrough.html")
    for out in (out1, out2):
        with open(out, "w", encoding="utf-8") as f:
            f.write(html)
        print("wrote", out, f"({len(html)} bytes)")
    print(f"enterprises={len(data['enterprises'])} rules={len(data['rules'])} "
          f"standards={len(data['standards'])} groups={len(data['groups'])}")


# The HTML template is imported from a sibling module to keep this file readable.
